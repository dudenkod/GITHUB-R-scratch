...
library(DMwR);library(MASS); library(randomForest)
...
train_caret= knnImputation(train_caret[,-c(18)], k=75)
valid_caret = knnImputation(valid_caret[,-c(18)], k=25)
...
train_caret = downSample(train_caret[,-18], train_caret$classLabel, yname = "classLabel")
...
model_RF = train(classLabel~., data=train_caret[,-c(12,17)], method="rf", preProcess="range", trControl=control, tuneGrid = expand.grid(.mtry = c(2:6)),  n.tree=188, maxit=2000, importance=TRUE)
...
model_GBM = train(classLabel~., data=train_caret[,-c(12,17)], method="gbm", trControl=control, tuneGrid=gbmGrid)

PREDICTION = predict(model_RF, newdata=valid_caret[,-c(12,17,18)])

PREDICTION = predict(model_GBM, newdata=valid_caret[,-c(12,17,18)])

confusionMatrix(PREDICTION,valid_caret[,18])
...
qq=as.data.frame(predict(my_tree_two, valid_caret[,-c(12,17,18)]))
colnames(qq) = c("NO","YES")
qq
answer = as.data.frame(qq$NO<qq$YES)
answer_prop = ifelse(answer==FALSE, "no.", "yes.")
colnames(answer_prop) = "classe"
qplot(answer_prop, colour=valid_caret[,18])

confusionMatrix(answer_prop,valid_caret[,18])
```
