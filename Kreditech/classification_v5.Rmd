...
modeFit = train(classLabel~., data=training[,-c(11,16,15,7,12,17)], method='glm', preProcess = NULL)
#modeFit = train(classLabel~., data=training[,-c(11,16,12,17)], method='knn', trControl=control, preProcess=c("center", "scale"), tuneLength = 10)
#modeFit = train(classLabel~., data=training[,-c(11,16,12,17)], method='nnet', preProcess="range", tuneLength = 3, trace = FALSE, maxit=100)
MM = predict(modeFit, training[,-c(11,16,15,7,12,17,19)])
UU = predict(modeFit, testing[,-c(11,16,12,15,7,17,19)])

confusionMatrix(MM,training[,19])
confusionMatrix(UU,testing[,19])

ZZ = predict(modeFit, valid_caret[,-c(11,16,15,7,12,17,19)])
