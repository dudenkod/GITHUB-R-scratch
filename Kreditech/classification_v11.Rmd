...
model_RF = train(classLabel~., data=train_caret[,!names(train_caret) %in% c("v35")], method="rf", preProcess="range", trControl=control, tuneGrid = expand.grid(.mtry = c(2:6)),  n.tree=188, maxit=2000, importance=TRUE)
...
PREDICTION = predict(model_RF, newdata=valid_caret[,!names(valid_caret) %in% c("v35","classLabel")])

PREDICTION = predict(model_GBM, newdata=valid_caret[,!names(train_caret) %in% c("v7","v18","classLabel")])

confusionMatrix(PREDICTION,valid_caret[1:198,"classLabel"])
...
my_tree_two <- rpart(classLabel ~., data = train_caret[,!names(train_caret) %in% c("v7","v18")], method = "class", control = rpart.control(minsplit = 50, cp = 0))
...
qq=as.data.frame(predict(my_tree_two, valid_caret[,!names(valid_caret) %in% c("v7","v18","classLabel")]))
