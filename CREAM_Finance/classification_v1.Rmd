...
train$target2 = NULL
summary(train)
train_v2=train[!is.na(train$ipaddress),]
train_v2_noIP = select(train,-ipaddress)

qq = train_v2
qq$ipaddress[is.na(qq$ipaddress)] <- "EMPTY"


#Downsampling
train_v3_noIP = downSample(train_v2_noIP[,!names(train_v2_noIP) %in% "target"], train_v2_noIP$target, yname = "target")
summary(train_v3_noIP)
inTrain = createDataPartition(y=train_v3_noIP$target, p=0.75, list = FALSE)
train_v3_noIP_train = train_v3_noIP[inTrain,]
train_v3_noIP_test = train_v3_noIP[-inTrain,]


control <- trainControl(method="cv", number=10, repeats=5, classProbs = TRUE)


model_RF = train(target~., data=train_v3_noIP_train, method="rf", preProcess="range", trControl=control, tuneGrid = expand.grid(.mtry = 4),  n.tree=188, maxit=2000, importance=TRUE, metric="ROC", proximity=TRUE)

PREDICTION = predict(model_RF, newdata=train_v3_noIP_test)

confusionMatrix(PREDICTION,train_v3_noIP_test[,"target"])
...
train_nsv = nearZeroVar(train_v2_noIP, saveMetrics = TRUE)
train_nsv

correlations <- cor(train_v2_noIP[sapply(train_v2_noIP,is.numeric)], use="pairwise", method="spearman")
